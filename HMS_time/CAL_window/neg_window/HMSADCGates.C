...
void HMSADCGates::SlaveBegin(TTree * /*tree*/)
{
  ...
  Bool_t HMSADCGates::Process(Long64_t entry)
  {
    ...
    if(entry<10000000) {
      ...
      h1PHGCERPulseTimeStarttimeDiff[current_pmt]->Fill(-H_cal_1pr_negAdcPulseTime[i] + *H_hod_starttime); // no selection adctdc time diff
      
      if(*H_cer_npeSum >2.0)// all hits with electron 
	{
	  h1PHGCERPulseTimeStarttimeDiff_CalCut[current_pmt]->Fill(-H_cal_1pr_negAdcPulseTime[i] + *H_hod_starttime);  // adctdc time diff with mult electron
	  ...
	  h1goodTimeCal[current_pmt]->Fill(H_cal_1pr_goodNegAdcTdcDiffTime[i]); //
	}
      ...
      h1PHGCERPulseTimeVsPulseAmpMultCut[current_pmt]->Fill(H_cal_1pr_goodNegAdcTdcDiffTime[i], H_cal_1pr_goodNegAdcPulseAmp[i]);
      
      if (H_cal_1pr_goodNegAdcMult[current_pmt] == 1) //hits with mult
	{
	  h1PHGCERPulseTimeStarttimeDiffMult[current_pmt]->Fill(-H_cal_1pr_negAdcPulseTime[i] + *H_hod_starttime);
	  
	  h1PHGCERPGoodAdcTdcDiffTime_Mult[current_pmt]->Fill(H_cal_1pr_goodNegAdcTdcDiffTime[current_pmt]);
	  
	  h1goodTimeVsAmpMult[current_pmt]->Fill(H_cal_1pr_goodNegAdcTdcDiffTime[i], H_cal_1pr_goodNegAdcPulseAmp[i]);
	  ...
	  if(*H_cer_npeSum >2.0)//hits with electron and Multiplicity
	    {
	      ...
	      h1goodTimeMultCal[current_pmt]->Fill(H_cal_1pr_goodNegAdcTdcDiffTime[i]);
	    }
	} 
