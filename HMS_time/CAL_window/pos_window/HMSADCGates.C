...
void HMSADCGates::SlaveBegin(TTree * /*tree*/)
{
  ...
  Bool_t HMSADCGates::Process(Long64_t entry)
  {
    ...
    if(entry<1000) {
      ...
      h1PHGCERPulseTimeStarttimeDiff[current_pmt]->Fill(-H_cal_1pr_posAdcPulseTime[i] + *H_hod_starttime); // no selection adctdc time diff
      
      if(*H_cer_npeSum >2.0)// all hits with electron 
	{
	  h1PHGCERPulseTimeStarttimeDiff_CalCut[current_pmt]->Fill(-H_cal_1pr_posAdcPulseTime[i] + *H_hod_starttime);  // adctdc time diff with mult electron
	  ...
	  h1goodTimeCal[current_pmt]->Fill(H_cal_1pr_goodPosAdcTdcDiffTime[i]); //
	}
      ...
      h1PHGCERPulseTimeVsPulseAmpMultCut[current_pmt]->Fill(H_cal_1pr_goodPosAdcTdcDiffTime[i], H_cal_1pr_goodPosAdcPulseAmp[i]);
      
      if (H_cal_1pr_goodPosAdcMult[current_pmt] == 1) //hits with mult
	{
	  h1PHGCERPulseTimeStarttimeDiffMult[current_pmt]->Fill(-H_cal_1pr_posAdcPulseTime[i] + *H_hod_starttime);
	  
	  h1PHGCERPGoodAdcTdcDiffTime_Mult[current_pmt]->Fill(H_cal_1pr_goodPosAdcTdcDiffTime[current_pmt]);
	  
	  h1goodTimeVsAmpMult[current_pmt]->Fill(H_cal_1pr_goodPosAdcTdcDiffTime[i], H_cal_1pr_goodPosAdcPulseAmp[i]);
	  ...
	  if(*H_cer_npeSum >2.0)//hits with electron and Multiplicity
	    {
	      ...
	      h1goodTimeMultCal[current_pmt]->Fill(H_cal_1pr_goodPosAdcTdcDiffTime[i]);
	    }
	} 
...
void HMSADCGates::Terminate()
{
  ...
  for (Int_t ipmt=0; ipmt < 13; ipmt++) {
    ...
    c1->SaveAs(Form("tight_CER_pmt_%i.pdf", ipmt+1));
