...
void legend(TH1F *histo0, TH1F *histo1,TH1F *histo2, TH1F *histo3)
{
  ...
  leg = new TLegend(0.69,0.83,0.9,0.95);
  leg->SetFillColor(-1);
  leg->SetFillStyle(0);
...
// ofstream phodo_neg_min("phodo_neg_min.txt", fstream::app);
// ofstream phodo_neg_max("phodo_neg_max.txt", fstream::app);

// ofstream phodo_pos_min("phodo_pos_min.txt", fstream::app);
// ofstream phodo_pos_max("phodo_pos_max.txt", fstream::app);
...
int main(){
  ...
  //TString fname = "../../../../hallc-replay-f2xem/ROOTfiles/shms_replay_production_all_wide_old_2548_-1.root";

  TString fname = "../../../NGCER_window/shms_replay_production_all_wideCut_2540_3000000.root";
  ...
  const Double_t planeOffset[nplanes]    = {1860., 2000., 2060., 2000.};
  ...
  for(UInt_t ip = 0; ip < nplanes; ip++) {
	...
	for(UInt_t isignal = 0; isignal < nsignals; isignal++) {
	  ...
	  if(signals[isignal] == "Adc") {
	    ...
	    chain.SetBranchAddress( error_flag, &errorFlag[ip][iside][0]);

	    TString good_adc_mult              = adc_base_name_good + "Mult";
	    chain.SetBranchAddress(good_adc_mult, &goodAdcMult[ip][iside][0]);
	    // cout << adc_mult << endl;

	    TString good_AdcTdcDiff            = adc_base_name_good + "TdcDiffTime";
	    chain.SetBranchAddress(good_AdcTdcDiff, &goodAdcTdcDiff[ip][iside][0]);

	    // cout << goodAdcTdcDiff << endl;
	    //  cout << adc_mult << endl;
	    
	    TString good_adc_neg_pulse_amp     = adc_base_name_good + "PulseAmp";
	    chain.SetBranchAddress(good_adc_neg_pulse_amp, &goodPulseAmp[ip][iside][0]);
  ...
  Double_t pos_difft[400], neg_difft[400], tdc_ref_time, adc_ref_time, tdc_ref_time2yt;
  ...
  TH2F* diff_neg_tdc[4][20];
  TH2F* diff_pos_tdc[4][20];
  
  TH1F* diff_neg_tdc_1d[4][20];
  TH1F* diff_pos_tdc_1d[4][20];
  TH1F* diff_pos_tdc_1d_cut[4][20];
	...
	//if (ip == 0){
	  ...
	  for(Int_t adc_neg_ihit = 0; adc_neg_ihit < nadchits[ip][0]; adc_neg_ihit++){
		...
		if ( adc_neg_bar == pmt_neg &&  tdc_neg_bar == pmt_neg ){
		  ...
		  if (cal>0.8 && ngcerNpe>2.0)                // electron cut
		    {
	...
	//for (int pmt = 0; pmt<1 ; pmt++)
	  {
	    ...
	    ////=============================================================================================================//
	    c1[i][pmt]->cd(3);
	    formatit(good_neg[i][pmt]," ",Form("P.hod.1x.GoodNegAdcTdcDiffTime_%d(ns)",pmt+1),"Events", 0);
	    ...
	    c1[i][pmt]->SaveAs(Form("zoomed_plots/Neg_S1X_plane_%d_Pmt_%d_test.pdf", i+1, pmt+1));
	    ...
	    c2[i][pmt]->cd(3);
	    formatit(good_pos[i][pmt]," ",Form("P.hod.1x.GoodPosAdcTdcDiffTime_%d(ns)",pmt+1),"Events", 0);
	    ...
	    c2[i][pmt]->SaveAs(Form("zoomed_plots/Pos_S1X_Plane_%d_Pmt%d_test.pdf", i+1, pmt+1));
