...
void SHMSADCGates::SlaveBegin(TTree * /*tree*/)
{
  ...
  for (Int_t ipmt=0; ipmt<4; ipmt++) {
    h1PHGCERPulseTime[ipmt] = new TH1F(Form("PHGCERPulseTime%d",ipmt+1),"", 1000,-100,100);
    ...
    h1PHGCERPGoodAdcTdcDiffTime[ipmt] = new TH1F(Form("PHGCERGoodAdcTdcDiffTime%d",ipmt+1),"", 400, -100, 60);// good with no cut

    h1PHGCERPGoodAdcTdcDiffTime_Mult[ipmt] = new TH1F(Form("PHGCERGoodAdcTdcDiffTimeMult%d",ipmt+1), "", 400, -100, 60); //good with mulltiplicity 
    ...
    h1goodTimeCal[ipmt] = new TH1F (Form("h1goodTimecal%d", ipmt+1),"", 400, -100, 60); //  good calorimeter
    
    h1goodTimeMultCal[ipmt] = new TH1F (Form("h1goodTimeMultCal%d", ipmt+1),"", 400, -100, 60); // good  multiplicity with calorimeter 
...
Bool_t SHMSADCGates::Process(Long64_t entry)
{
  ...
  if(entry<100000000) {//Abort("end of the desired entry");
	...
	if(*P_cal_etottracknorm >0.8)// all hits with electron 
	  {
	    ...
	    h1goodTimeCal[current_pmt]->Fill(P_ngcer_goodAdcTdcDiffTime[i]); //
	  }
	...
	h1PHGCERPulseTimeVsPulseAmpMultCut[current_pmt]->Fill(P_ngcer_goodAdcTdcDiffTime[i], P_ngcer_goodAdcPulseAmp[i]);
	...
	if (P_ngcer_goodAdcMult[current_pmt] == 1) //hits with mult
	  {
	    ...
	    h1goodTimeVsAmpMult[current_pmt]->Fill(P_ngcer_goodAdcTdcDiffTime[i], P_ngcer_goodAdcPulseAmp[i]);
		...
		h1goodTimeMultCal[current_pmt]->Fill(P_ngcer_goodAdcTdcDiffTime[i]);
	      }
	  } 
	...
	if (P_ngcer_goodAdcTdcDiffTime[i]> -50 && P_ngcer_goodAdcTdcDiffTime[i]<-30)// small peak looking into good variable
	  {
	    ...
	    h1weightAmp->Fill(*P_dc_x_fp - 89.1 * (*P_dc_xp_fp), *P_dc_y_fp - 89.1 *(*P_dc_yp_fp), P_ngcer_goodAdcPulseAmp[i]); //weighting by Amplitude

	    h1trackproject->Fill(*P_dc_x_fp - 89.1 * (*P_dc_xp_fp), *P_dc_y_fp -89.1 *(*P_dc_yp_fp)); 
	  }

	if (P_ngcer_goodAdcTdcDiffTime[i]> -30 && P_ngcer_goodAdcTdcDiffTime[i] <-10)// big peak
	  {
...
void SHMSADCGates::Terminate()
{
  ...
  //   legend[ipmtt]->Draw();
    ...
    // c1->SaveAs(Form("AdcTdc_NGCER_pmt_%i.png", ipmt+1));
    ...
    //delete [] legend;
  // }
  // *********************good varible histo plots with zoomed **************************//
  ...
  for (int ipmt =0; ipmt<4; ipmt++)
    {
      ...
      h1PHGCERPGoodAdcTdcDiffTime[ipmt]->Draw();//gPad->SetLogz(1);
      h1PHGCERPGoodAdcTdcDiffTime[ipmt]->GetXaxis()->SetTitle("P.ngcer.goodAdcTdcDiffTime");
      h1PHGCERPGoodAdcTdcDiffTime[ipmt] ->GetYaxis()->SetTitle("Counts");
      ...
      auto * line =  new TLine (-49, 0, -49, 11000);
      line->Draw();
      line->SetLineColor(kMagenta+4);

      auto * line1 =  new TLine (-12, 0, -12, 11000);
      line1->Draw();
      line1->SetLineColor(kMagenta+4);

    }
  c0->SaveAs("zommed_plot.png");
  ...
  // }
  //  c2->SaveAs("Ngcer_Amplitude_pmt.png");
  //  c3->SaveAs("Ngcer_Integral_pmt.png");
  ...
  //     rp->GetLowerRefGraph()->GetXaxis()->SetRangeUser(-100,50);
  ...
  //     rp1->GetLowerRefGraph()->GetXaxis()->SetRangeUser(-100,50);
      
  //   }
  // c4->Update(); c4->SetTicks(0, 1);
  // c4->SaveAs("Ngcer_Blue_ratio_plot.png"); 
  // c5->Update(); c5->SetTicks(0, 1);
  // c5->SaveAs("Ngcer_Red_ratio_plot.png");
  ...
  //     h1XtrackVsYtrack[i]->Draw();
  //   }
  // c6->SaveAs("Ngcer_track_cer.png");
