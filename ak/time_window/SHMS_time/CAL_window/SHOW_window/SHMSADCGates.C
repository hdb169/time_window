...
void SHMSADCGates::SlaveBegin(TTree * /*tree*/)
{
  ...
  h1PHGCERPulseTimeStarttimeDiff_CalCut = new TH1F*[256];
  
  //****************** HgCer( 4 pmts) **********************************//
  h1PHGCERPulseTime              = new TH1F*[256];
  h1PHGCERPulseTimeStarttimeDiff = new TH1F*[256];
  h1PHGCERPGoodAdcTdcDiffTime = new TH1F*[256];
  h1PHGCERPGoodAdcTdcDiffTime_Mult = new TH1F*[256];
  h1PHGCERPulseTimeVsPulseAmp = new TH2F*[256];
  h1PHGCERPulseTimeStarttimeDiffMult = new TH1F*[256];
  h1PHGCERPulseTimeStarttimeDiffMultCut = new TH1F*[256];
  h1PHGCERPulseTimeVsPulseAmpMultCut = new TH2F*[256];
  h1TimeVsAmpMult =new TH2F*[256];
  h1TimeVsAmpCal = new TH2F*[256];
  h1TimeVsAmpMultCal=new TH2F*[256];
  h1goodTimeVsAmpMult=new TH2F*[256];

  h1goodTimeCal= new TH1F*[256];
  h1goodTimeMultCal= new TH1F *[256];
  h1_gooddelta =new TH1F*[256];
  h1_delta =new TH1F*[256];

  for (Int_t ipmt=0; ipmt<256; ipmt++) {
...
Bool_t SHMSADCGates::Process(Long64_t entry)
{
  ...
  if(entry<100000000) { //Abort("end of the desired entry");
    ...
    for (Int_t i = 0; i < *Ndata_P_cal_adcPulseTime; i++) {
      ...
      h1PHGCERPulseTime[current_pmt]->Fill(-P_cal_adcPulseTime[i]);
      
      h1PHGCERPulseTimeStarttimeDiff[current_pmt]->Fill(-P_cal_adcPulseTime[i] + *P_hod_starttime); // no hit selection. Its adctdc time diff

      if(*P_ngcer_npeSum > 2.0)// all hits with electron 
	{
	  if(*P_gtr_dp>-15 && *P_gtr_dp<25)
	    {
	      h1_delta[current_pmt]->Fill(-P_cal_adcPulseTime[i] + *P_hod_starttime); //delta cut on no hit selection
	      
	      h1_gooddelta[current_pmt]->Fill(P_cal_goodAdcTdcDiffTime[current_pmt]); //delta cut on hit selection
	    }
	  
	  h1PHGCERPulseTimeStarttimeDiff_CalCut[current_pmt]->Fill(-P_cal_adcPulseTime[i] + *P_hod_starttime);  // adctdc time diff with multiplicity cut and calorimter cut
	  
	  h1TimeVsAmpCal[current_pmt]->Fill(-P_cal_adcPulseTime[i] + *P_hod_starttime, P_cal_adcPulseAmp[i]); // amplitude with electron
	  
	  h1goodTimeCal[current_pmt]->Fill(P_cal_goodAdcTdcDiffTime[current_pmt]); 
	}
      
      h1PHGCERPulseTimeVsPulseAmp[current_pmt]->Fill(-P_cal_adcPulseTime[i] + *P_hod_starttime,  P_cal_adcPulseAmp[i]);
      
      h1PHGCERPGoodAdcTdcDiffTime[current_pmt]->Fill(P_cal_goodAdcTdcDiffTime[current_pmt]);
      
      h1PHGCERPulseTimeVsPulseAmpMultCut[current_pmt]->Fill(P_cal_goodAdcTdcDiffTime[current_pmt], P_cal_goodAdcPulseAmp[current_pmt]); // hit selection with no cuts
      
      if (P_cal_goodAdcMult[current_pmt] == 1) //hits with mult
	{
	  h1PHGCERPulseTimeStarttimeDiffMult[current_pmt]->Fill(-P_cal_adcPulseTime[i] + *P_hod_starttime);// no hit selection with multiplicity cut
	  
	  h1PHGCERPGoodAdcTdcDiffTime_Mult[current_pmt]->Fill(P_cal_goodAdcTdcDiffTime[current_pmt]);
	  
	  h1goodTimeVsAmpMult[current_pmt]->Fill(P_cal_goodAdcTdcDiffTime[current_pmt], P_cal_goodAdcPulseAmp[current_pmt]);
	  
	  h1TimeVsAmpMult[current_pmt]->Fill(-P_cal_adcPulseTime[i] + *P_hod_starttime, P_cal_adcPulseAmp[i]);
	  
	  if(*P_ngcer_npeSum >2.0)//hits with electron and Multiplicity
	    {
	      h1PHGCERPulseTimeStarttimeDiffMultCut[current_pmt]->Fill(-P_cal_adcPulseTime[i] + *P_hod_starttime);// all hit with elec and mult==1
	      
	      h1TimeVsAmpMultCal[current_pmt]->Fill(-P_cal_adcPulseTime[i] + *P_hod_starttime,  P_cal_adcPulseAmp[i]);
	      
	      h1goodTimeMultCal[current_pmt]->Fill(P_cal_goodAdcTdcDiffTime[current_pmt]);
	    }   
	}  
    }
...
void SHMSADCGates::Terminate()
{
  ...
  for (Int_t ipmt=0; ipmt < 256; ipmt++) {
    ...
    h1PHGCERPulseTimeStarttimeDiff_CalCut[ipmt]->GetXaxis()->SetTitle("-P.cal.pr.adcPulseTime + P.hod.starttime");
    ...
    h1PHGCERPulseTimeStarttimeDiff[ipmt]->GetXaxis()->SetTitle("-P.cal.pr.adcPulseTime + P.hod.starttime");
    ...
    legend[ipmtt]->AddEntry(h1PHGCERPulseTimeStarttimeDiff_CalCut[ipmt],"Cer >2.0","l");
    legend[ipmtt]->AddEntry(h1PHGCERPulseTimeStarttimeDiffMultCut[ipmt],"mult = 1 & Cer","l");
    legend[ipmtt]->AddEntry(h1_delta[ipmt],"#delta >-15 && #delta <25 && Cer","l");
    ...
    h1PHGCERPulseTimeVsPulseAmp[ipmt]->GetXaxis()->SetTitle("-P.cal.pr.adcPulseTime + P.hod.starttime");
    h1PHGCERPulseTimeVsPulseAmp[ipmt]->GetYaxis()->SetTitle("P.cal.pr.adcPulseAmp");
    ...
    h1TimeVsAmpMult[ipmt ]->GetXaxis()->SetTitle("-P.cal.pr.adcPulseTime + P.hod.starttime");
    h1TimeVsAmpMult[ipmt ] ->GetYaxis()->SetTitle("P.cal.pr.adcPulseAmp");
    ...
    h1TimeVsAmpCal[ipmt ]->GetXaxis()->SetTitle("-P.cal.pr.adcPulseTime + P.hod.starttime");
    h1TimeVsAmpCal[ipmt ] ->GetYaxis()->SetTitle("P.cal.pr.adcPulseAmp");
    ...
    h1TimeVsAmpMultCal[ipmt ]->GetXaxis()->SetTitle("-P.cal.pr.adcPulseTime+ P.hod.starttime");
    h1TimeVsAmpMultCal[ipmt ] ->GetYaxis()->SetTitle("P.cal.pr.adcPulseAmp");
    ...
    legend[ipmtt]->Draw();
    //  delete legend[ipmtt];
   c1->SaveAs(Form("Plots/CAL_AdcTdc_pmt_%i.pdf", ipmt+1));
  ...
  for (Int_t ipmt=0; ipmt < 256; ipmt++) {
    ...
    h1PHGCERPGoodAdcTdcDiffTime[ipmt]->GetXaxis()->SetTitle("P.cal.pr.goodAdcTdcDiffTime");
    ...
    h1PHGCERPulseTimeVsPulseAmpMultCut[ipmt]->GetXaxis()->SetTitle("P.cal.pr.goodAdcTdcDiffTime");
    h1PHGCERPulseTimeVsPulseAmpMultCut[ipmt]->GetYaxis()->SetTitle("P.cal.pr.goodAdcPulseAmp");
    ...
    c0->SaveAs(Form("Plots/Good_CAL_AdcTdc_pmt_%i.pdf", ipmt+1));
    delete c0;
